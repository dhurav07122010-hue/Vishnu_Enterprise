...
const OWNER_EMAIL = process.env["GMAIL_USER"]; // notifications go to the same Gmail
...
export async function sendOrderConfirmation(order: OrderEmailData) {
  ...
  logger.info({ orderNumber: order.order_number }, "Order confirmation emails sent");
}
...
export async function sendOrderStatusUpdate(order: OrderEmailData) {
  ...
  logger.info({ orderNumber: order.order_number }, "Order status update emails sent");
}
